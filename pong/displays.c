...
static void ascii_set_bits(unsigned char bits)
{
    *portE_odr_lo |= (CTRL_SELECT | bits);
}

static void ascii_clear_bits(unsigned char bits)
{
    bits = ~bits;
    bits |= CTRL_SELECT;
    *portE_odr_lo &= bits;
...
{
    ascii_set_bits(CTRL_E);
    *portE_odr_hi = c;
    ascii_clear_bits(CTRL_E);
    delay_250ns();
}
...
{
    // Rs=0, RW=0
    ascii_clear_bits(CTRL_RS | CTRL_RW);
...
{
    // RS=1, RW=0
    ascii_set_bits(CTRL_RS);
    ascii_clear_bits(CTRL_RW);
...
static unsigned char ascii_read_status(void)
{
    ...
    *portE_moder = 0x00005555;
    ...
    // RS=0, RW=1
    ascii_set_bits(CTRL_RW);
    ascii_clear_bits(CTRL_RS);
...
static unsigned char ascii_read_data(void)
{
    ...
    *portE_moder = 0x00005555;
    ...
    // RS=1, RW=1
    ascii_set_bits(CTRL_RW | CTRL_RS);
...
static void _lcd_write(unsigned char dataByte, unsigned char cs, int rs)
{
    ...
    graphic_ctrl_bit_clear(CTRL_E); // E=0
	select_controller(cs);
	if (rs)
        graphic_ctrl_bit_set(CTRL_RS); 
	else
        graphic_ctrl_bit_clear(CTRL_RS);
	graphic_ctrl_bit_clear(CTRL_RW); // rw=0
	// graphic write
	*portE_odr_hi = dataByte;
    ...
    //*portE_odr_hi = dataByte;
    delay_500ns();
    graphic_ctrl_bit_set(CTRL_E); // E=1
    delay_500ns();
    graphic_ctrl_bit_clear(CTRL_E); // E=0
    ...
    *portE_odr_hi = 0;   // data = 0
    graphic_ctrl_bit_set(CTRL_E); // E=1
    graphic_ctrl_bit_clear(CTRL_CS_BOTH); // CS1=0,CS2=0
}
...
void lcd_init(void)
{
    ...
    *portE_pupdr = 0x55550000; /* inputs are pull up */
	...
	graphic_ctrl_bit_clear(CTRL_SELECT);
	
    graphic_ctrl_bit_set(CTRL_E); // E=1
    delay_micro(10);
    graphic_ctrl_bit_clear(CTRL_CS_BOTH|CTRL_RESET|CTRL_E);
    delay_milli(30);
    graphic_ctrl_bit_set(CTRL_RESET);
